...
def left_down(handle: HWND, x: int, y: int):
    ...
    wparam = 0
    lparam = y << 16 | x
    SendInput(handle, WM_LBUTTONDOWN, wparam, lparam)
...
def left_up(handle: HWND, x: int, y: int):
    ...
    wparam = 0
    lparam = y << 16 | x
    SendInput(handle, WM_LBUTTONUP, wparam, lparam)
...
#     import cv2
#     handle = windll.user32.FindWindowW(None, "Calculator")
#     # 点击线路
#     left_down(handle, 100, 520)
#     time.sleep(0.1)
#     left_up(handle, 100, 520)
#     time.sleep(1)
#     # 滚动线路列表
